...
CoapGateway.prototype.on_request = function (req, res) {
  ...
  try {
    ...
    //var ptr = 0;
    //// Get device id
    //var id_len = payload.readUInt8(ptr);
    //ptr++;
    //var device_id = payload.slice(ptr, ptr+id_len).toString('hex');
    //debug(device_id);
    //ptr += id_len;
    //var version = payload.readUInt8(ptr);
    //ptr++;
    //var seq_no = 0;
    //debug("version " + version.toString())
    //if (version > 1) {
    //  seq_no = payload.readUInt32LE(ptr);
    //  ptr+=4;
    //  debug("seq " + seq_no.toString())
    //}
    //var t_sec = bigInt(payload.readInt32LE(ptr)) + bigInt(payload.readInt32LE(ptr+32/8) << 32);
    //ptr += 64/8;
    //var t_usec = payload.readInt32LE(ptr);
    //ptr += 32/8;
    //payload = payload.slice(ptr);
